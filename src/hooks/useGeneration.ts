...
export function useGeneration<TInput, TOutput>(config: UseGenerationConfig<TInput, TOutput>) {
  ...
  const inFlightGenerationRef = useRef(false)
  ...
  const generate = useCallback(async (input: TInput) => {
    if (inFlightGenerationRef.current) {
      throw new Error(t.errors.generationInProgress)
    }

    inFlightGenerationRef.current = true
    ...
    try {
      ...
      if (modelSettingsResult.status === 'fulfilled') {
        ...
        if (modelSettings) {
          if (modelSettings.is_enabled === false && !isAdmin) {
            const modelDisabledError = t.errors.modelUnavailable
      ...
      if (!isAdmin) {
        if (planLimitResult.status === 'fulfilled') {
          ...
          if (
            ...
            const planLimitError = t.errors.concurrentLimitReached.replace('{limit}', String(planLimit.max_concurrent_generations))
        ...
        if (maintenanceModeResult.status === 'fulfilled') {
          if (maintenanceModeResult.value) {
            const maintenanceError = t.errors.maintenanceInProgress
      ...
      if (blacklistMatch) {
        const reason = t.errors.promptBlockedByPolicy
  ...
  // On mount (or when context generations change), reconnect to the most recent
  // generation of this type if the hook has no active generation yet.
  // This covers: navigating away and back, or the app restoring from DB.
  // Note: this effect comes AFTER restoreGeneration definition to avoid TDZ error.
  useEffect(() => {
    ...
    const matching = generations
      .filter(
        g =>
          g.type === config.generationType &&
          (g.status === 'starting' || g.status === 'processing' || g.status === 'succeeded'),
      )
      .sort((a, b) => b.createdAt - a.createdAt)[0]

    if (matching) {
      restoredRef.current = true
      restoreGeneration(matching.id)
    }
  }, [generations, state.generationId, config.generationType, restoreGeneration])
