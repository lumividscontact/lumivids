-- =============================================
-- Cap free credits to a hard maximum of 10 total.
--
-- Policy:
-- - New users receive only 10 free credits on signup.
-- - No automatic daily free top-up is granted.
-- - Freemium daily limit remains 10 credits/day (usage gate),
--   but free balance is never replenished automatically.
-- =============================================

-- Ensure freemium bookkeeping columns exist for compatibility.
...
BEGIN
  ...
  -- Admins bypass freemium restrictions.
  v_is_paid := v_is_paid OR v_is_admin;

  -- New users still start with at most 10 free credits.
  ...
  SELECT uc.credits, uc.free_daily_used, uc.free_daily_date
    INTO v_current_credits, v_current_used, v_current_date
  ...
  -- Remaining free usage is constrained by both daily cap and current balance.
  v_daily_remaining := LEAST(v_current_credits, GREATEST(v_quota - v_current_used, 0));
