-- =============================================
-- Admin users bypass freemium daily limit.
-- Admins are treated as paid users in both
-- refresh_freemium_credits and deduct_credits.
-- =============================================
...
BEGIN
  ...
  -- Admins are treated as paid (bypass freemium limit)
  ...
  IF v_current_date IS DISTINCT FROM v_today THEN
    ...
    ELSE
      -- Paid/admin users or free with enough credits: just reset usage counter.
      UPDATE public.user_credits uc
  ...
  SELECT
    ...
    NOT v_is_paid,  -- is_freemium = false for admins
    ((date_trunc('day', now() AT TIME ZONE 'utc') + interval '1 day') AT TIME ZONE 'utc')::timestamptz;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;


-- Update deduct_credits to also bypass freemium check for admins.
...
BEGIN
  ...
  -- Admins have is_freemium = false, so this check is skipped for them
